...
// ── Automatic sea & forest collage ─────────
function initSeaCollage() {
  const collage = document.getElementById('seaCollage');
  if (!collage) return;

  const photos = [
    {
      src: 'media/sea-forest-1.webp',
      alt: 'Морски залив до зелена гора',
      caption: 'Море, гора и златна светлина'
    },
    {
      src: 'media/sea-forest-2.webp',
      alt: 'Горска пътека към морето',
      caption: 'Пътека към спокойствието'
    },
    {
      src: 'media/sea-forest-3.webp',
      alt: 'Златен залез над морето и гората',
      caption: 'Залез край морето'
    }
  ];

  const slots = [
    {
      image: document.getElementById('seaCollageMain'),
      caption: document.getElementById('seaCollageMainCaption')
    },
    {
      image: document.getElementById('seaCollageSideOne'),
      caption: document.getElementById('seaCollageSideOneCaption')
    },
    {
      image: document.getElementById('seaCollageSideTwo'),
      caption: document.getElementById('seaCollageSideTwoCaption')
    }
  ];
  const dots = [...document.querySelectorAll('.sea-collage__dot')];
  ...
  function showPhoto(index) {
    active = index;
    collage.classList.add('is-switching');

    window.setTimeout(() => {
      slots.forEach((slot, position) => {
        const photo = photos[(active + position) % photos.length];
        slot.image.src = photo.src;
        slot.image.alt = photo.alt;
        slot.caption.textContent = photo.caption;
      });
      ...
      collage.classList.remove('is-switching');
    }, reduceMotion ? 0 : 260);
  }
  ...
  function startRotation() {
    ...
    timer = window.setInterval(() => showPhoto((active + 1) % photos.length), 5200);
  }
  ...
  collage.addEventListener('mouseenter', () => window.clearInterval(timer));
